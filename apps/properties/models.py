...
class PropertyPublishManeger(models.Manager):
    def get_queryset(self) :
        return (
            super(PropertyPublishManeger,self).get_queryset().filter(published_status=True)
        )
...
class Property(TimeStampUUIDModel):
    ...
    tax = models.DecimalField(verbose_name=_("Property Tax"), max_digits=7, decimal_places=2, default=0.15, help_text="15% property tax charged",)
    ...
    bathrooms = models.DecimalField(verbose_name=_("Bathrooms"), max_digits=5, decimal_places=2,default=1.0)
    advert_type = models.CharField(_("Advert Type"), max_length=50,choices=AdvertType.choices, default=AdvertType.FOR_SALE)
    property_type = models.CharField(_("Property Type"), max_length=50, choices=PropertyType.choices, default=PropertyType.OTHER)
    cover_photo = models.ImageField(verbose_name=_("Main Phote"), default="/house_sample.jpg",null=True,blank=True)
    ...
    published = PropertyPublishManeger()
...
class PropertyViews(TimeStampUUIDModel):
    ...
    property = models.ForeignKey(Property, related_name=_("Property_views"), on_delete=models.CASCADE)
